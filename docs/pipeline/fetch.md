...
| `GOOGLE_DOCS_FORMAT` | `md` | export format for native Google Docs. Changing it re-exports the affected files on the next pass (the skip compares the recorded filename against the one the current config would write, not just the remote fingerprint) |
